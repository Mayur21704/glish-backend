...
let db
try {
  // 1. Preferred: Node 22+/Node 24 native built-in SQLite (zero C++ dependencies, immune to SIGSEGV)
  const { DatabaseSync } = require('node:sqlite')
  const nativeDb = new DatabaseSync(DB_PATH)
  nativeDb.pragma = function (cmd) {
    if (cmd.startsWith('table_info(')) {
      return this.prepare(`PRAGMA ${cmd}`).all()
    }
    return this.exec(`PRAGMA ${cmd}`)
  }
  db = nativeDb
  console.log('✓ SQLite enterprise database connected via native node:sqlite:', DB_PATH)
} catch (nativeErr) {
  // 2. Fallback to better-sqlite3
  const Database = require('better-sqlite3')
  db = new Database(DB_PATH)
  console.log('✓ SQLite enterprise database connected via better-sqlite3:', DB_PATH)
}
...
module.exports = {
  ...
  saveSession: (session, userId) => {
    ...
    return { ...session, id: Number(result.lastInsertRowid), user_id: userId }
  },
  ...
  saveVocab: (item, userId) => {
    ...
    return { ...item, id: Number(result.lastInsertRowid), user_id: userId, mastered: Boolean(item.mastered) }
  },
